...
local account_max_items = 300;
local char_max_items = 120;
local tax = 0.10;
local tax_str = 10;
  ...
  row = cur:fetch (row, "a");
     ...
     total_paid = tonumber(total_paid) + tonumber(row.price);
      ...
      con:close();
        ...
        e.other:Message(6, "{Filters}: [".. eq.say_link("class+",false,"show items by class") .."] # [".. eq.say_link("spells+",false,"spells and tomes") .."] # [".. eq.say_link("search+dose",false,"potions") .."]");
...
--    e.self:Emote("1");
    e.other:Message(8, "The command is [Commit+ItemName+PriceinPlatinum] (example: /say commit+paw of opalla+500 or use direct+ for contact only sale.). For potions and expendables with charges use the command with the item on your cursor [Pot+ItemName+PriceinPlatinum] (example /say pot+5 Dose Bixie Sting+100). You will be taxed "..tostring(tax_str).. " percent on all sales. Post up to 30 items per character. Use item links! Items expire after 12 weeks and spells after 24 weeks.");
...
if (e.message:findi("OnSale")) then
  ...
  local repost_instruct = split(e.message, " ");
  local repost_info = con:execute("SELECT ID,Item_ID,Price,Char_Name,Item_Name,Sale_Type,Aug_Num FROM item_auction WHERE Active = 1 AND ID = '"..repost_instruct[2].."'");
  local row3 = repost_info:fetch ({}, "a");
  if (row3 == nil) then
      e.other:Message(6, "Doesn't exist or is not expired.");
      return
...
con:close();
    ...
    if (e.message:findi("pot+")) then
        ...
        local row6 = item_find:fetch ({}, "a");

        if(row6) then
            
        else
            e.other:Message(6, "I find no such item on your cursor.");
            return
        end
